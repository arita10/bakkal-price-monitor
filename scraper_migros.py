"""
scraper_migros.py — Migros scraper (migros.com.tr)
...
logger = logging.getLogger("bakkal_monitor.scraper_migros")
...
def _parse_tr_price(raw: str) -> float:
    ...
    Handles cases like:
      ...
      '1.249,90 TL'     -> 1249.90
    """
    # Extract last price-like pattern (handles "label\nprice" format)
...
def _page_url(base: str, page_num: int) -> str:
    """Return URL for the given page number (page 1 = no param)."""
    if page_num <= 1:
        return base
    # Append ?sayfa=N (or &sayfa=N if there are already params)
    sep = "&" if "?" in base else "?"
    return f"{base}{sep}sayfa={page_num}"


async def scrape_migros_direct() -> list:
    """
    Scrape all MIGROS_TARGET_URLS using Playwright.
    ...
    async with async_playwright() as p:
        ...
        for base_url in MIGROS_TARGET_URLS:
            url_total = 0
            try:
                ...
                for page_num in range(1, MAX_PAGES + 1):
                    ...
                    for card in cards:
                        try:
                            ...
                            # Prefer sale-price (discounted), fallback to any price
                    ...
                    logger.info(
                        f"Migros: page {page_num} of {base_url} -> {page_count} products"
                    )

                    # If this page returned fewer than 30 cards it's the last page
                    if len(cards) < 30:
                        break
