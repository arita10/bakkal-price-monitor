"""
scraper_sok.py — SOK Market scraper (sokmarket.com.tr)
...
Max 10 pages per category (safety cap — most categories fit in 1-2 pages).
"""
...
logger = logging.getLogger("bakkal_monitor.scraper_sok")
...
def _parse_tr_price(raw: str) -> float:
    """
    Convert SOK price string to float.
    '17,90₺'     -> 17.90
    '1.249,90₺'  -> 1249.90
    """
...
async def scrape_sok_direct() -> list:
    """
    Scrape all SOK_TARGET_URLS using Playwright.
    Products load fully on page load (no infinite scroll).
    ...
    async with async_playwright() as p:
        ...
        for base_url in SOK_TARGET_URLS:
            url_total = 0
                ...
                for page_num in range(1, MAX_PAGES + 1):
                    ...
                    for card in cards:
                        try:
                            ...
                            # Link is on the wrapping <a> parent of the card div
                            ...
                            # Detect page wrap-around (same first product as page 1)
                            if page_num == 1 and page_count == 0:
                                ...
                                if name == first_product_page1:
                                    ...
                                    cards = []  # signal outer loop to break
                                    break
