...
Handles:
  get_last_price()   — read most recent price from price_history
  get_price_history() — read N days of history for a product
  get_best_deals()   — today's biggest price drops
  upsert_price()     — write to products + price_history tables
"""
...
def get_last_price(supabase: Client, product_url: str) -> Optional[float]:
    """
    Return the most recently recorded price for product_url,
...
def upsert_price(
    ...
    """
    1. Upsert into `products` table (master product registry).
    2. Upsert into `price_history` table (daily time-series).
